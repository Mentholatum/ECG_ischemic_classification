...
本项目针对 12 导联心电图（ECG）的缺血性病变检测任务，提出了一种融合 **多尺度卷积初始层**、**导联间关系自注意力** 与 **多尺度时序金字塔** 的深度网络架构 `ECGNet_v3`。项目支持从原始 WFDB 格式数据到模型训练、五折交叉验证的完整流程，并内置了丰富的数据增强与早停机制，适用于高阳性率（~70%）的临床 ECG 数据集。
...
```
======================================================================
  Fold 0/5 | Train=others, Val=Test=Fold 0
======================================================================
...
[Fold 0] | Loss: 0.2341 | F1=0.8123 AUC=0.9012 Score=1.6786 | -> BEST
...
======================================================================
  5-Fold Summary
======================================================================
  Fold 0: F1=0.8123 | AUC=0.9012 | Acc=0.8234 | BalAcc=0.8123 | Sens=0.8345 | Spec=0.7901
  ...

  Mean±Std: F1=0.8156±0.0032 | AUC=0.8998±0.0021 | BalAcc=0.8145±0.0028 | Spec=0.7934±0.0041
```
...
`ECGNet_v3` 是专为 12 导联 ECG 信号设计的深度分类网络，整体架构遵循 **"局部形态提取 → 导联关系建模 → 多尺度时序融合 → 深层判别"** 的层次化设计思想。
...
将 256 维通道特征映射到 12 个导联的语义空间（每导联 16 维），通过 `MultiheadAttention` 学习导联间的全局依赖关系（如 V1-V3 对前壁心梗的协同响应、肢体导联与胸导联的电传导耦合），再映射回通道空间，实现可解释的导联间信息交互。
